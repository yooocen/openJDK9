    ...
    if (mem_ptr == NULL) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          log_trace(jvmti)("[%s] %s {  size=" INT64_FORMAT "", curr_thread_name, func_name, size);
      }
  ...
  if (thread == NULL) {
    ...
    if (thread_oop == NULL) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        ...
        log_error(jvmti)("[%s] %s } %s - erroneous arg is thread - jthread resolved to NULL - jthread = " PTR_FORMAT "",  curr_thread_name, func_name, 
                  JvmtiUtil::error_name(JVMTI_ERROR_INVALID_THREAD), p2i(thread));
      }
    ...
    if (!thread_oop->is_a(SystemDictionary::Thread_klass())) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        ...
        log_error(jvmti)("[%s] %s } %s - erroneous arg is thread - oop is not a thread - jthread = " PTR_FORMAT "",  curr_thread_name, func_name, 
                  JvmtiUtil::error_name(JVMTI_ERROR_INVALID_THREAD), p2i(thread));
      }
      return JVMTI_ERROR_INVALID_THREAD;
    }
    ...
    if (java_thread == NULL) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        ...
        log_error(jvmti)("[%s] %s } %s - erroneous arg is thread - not a Java thread - jthread = " PTR_FORMAT "",  curr_thread_name, func_name, 
                  JvmtiUtil::error_name(JVMTI_ERROR_THREAD_NOT_ALIVE), p2i(thread));
      }
...
jvmtiTrace_SuspendThreadList(jvmtiEnv* env,
  ...
  if (results == NULL) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          log_trace(jvmti)("[%s] %s {  request_count=" INT32_FORMAT " request_list=" PTR_FORMAT "", curr_thread_name, func_name, request_count, 
      p2i(request_list)
    );
      }
...
jvmtiTrace_RunAgentThread(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  arg=" PTR_FORMAT " priority=" INT32_FORMAT "", curr_thread_name, func_name, 
      p2i(arg)
    , priority);
  }
...
jvmtiTrace_GetThreadListStackTraces(jvmtiEnv* env,
  ...
  if (max_frame_count < 0) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          log_trace(jvmti)("[%s] %s {  thread_count=" INT32_FORMAT " thread_list=" PTR_FORMAT "", curr_thread_name, func_name, thread_count, 
      p2i(thread_list)
    );
      }
        log_error(jvmti)("[%s] %s } %s - erroneous arg is max_frame_count",  curr_thread_name, func_name, 
  ...
  if (stack_info_ptr == NULL) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          log_trace(jvmti)("[%s] %s {  thread_count=" INT32_FORMAT " thread_list=" PTR_FORMAT " max_frame_count=" INT32_FORMAT "", curr_thread_name, func_name, thread_count, 
      p2i(thread_list)
    , max_frame_count);
      }
        log_error(jvmti)("[%s] %s } %s - erroneous arg is stack_info_ptr",  curr_thread_name, func_name, 
...
jvmtiTrace_ForceEarlyReturnLong(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  thread=%s value=" INT64_FORMAT "", curr_thread_name, func_name, 
                    JvmtiTrace::safe_get_thread_name(java_thread), value);
  }
...
jvmtiTrace_FollowReferences(jvmtiEnv* env,
  ...
  if (callbacks == NULL) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          log_trace(jvmti)("[%s] %s {  heap_filter=" INT32_FORMAT " klass=" PTR_FORMAT "", curr_thread_name, func_name, heap_filter, 
      p2i(klass)
    );
      }
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  heap_filter=" INT32_FORMAT " klass=" PTR_FORMAT " callbacks=" PTR_FORMAT " user_data=" PTR_FORMAT "", curr_thread_name, func_name, heap_filter, 
      p2i(klass)
    , 
  p2i(callbacks)
, 
...
jvmtiTrace_SetTag(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  tag=" INT64_FORMAT "", curr_thread_name, func_name, tag);
  }
...
jvmtiTrace_GetObjectsWithTags(jvmtiEnv* env,
  ...
  if (count_ptr == NULL) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          log_trace(jvmti)("[%s] %s {  tag_count=" INT32_FORMAT " tags=" PTR_FORMAT "", curr_thread_name, func_name, tag_count, 
      p2i(tags)
    );
      }
        log_error(jvmti)("[%s] %s } %s - erroneous arg is count_ptr",  curr_thread_name, func_name, 
...
jvmtiTrace_IterateOverObjectsReachableFromObject(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  user_data=" PTR_FORMAT "", curr_thread_name, func_name, 
      p2i(user_data)
    );
  }
...
jvmtiTrace_SetLocalLong(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  thread=%sdepth=%d slot=" INT32_FORMAT " value=" INT64_FORMAT "", curr_thread_name, func_name, 
                    JvmtiTrace::safe_get_thread_name(java_thread), depth, slot, value);
  }
...
jvmtiTrace_SetBreakpoint(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  method=%s.%s location=" INT64_FORMAT "", curr_thread_name, func_name, 
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(3);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(3);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
...
static jvmtiError JNICALL
jvmtiTrace_GetNamedModule(jvmtiEnv* env,
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(40);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(40);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
...
static jvmtiError JNICALL
jvmtiTrace_AddModuleReads(jvmtiEnv* env,
            jobject module,
            jobject to_module) {
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(94);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(94);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
...
static jvmtiError JNICALL
jvmtiTrace_AddModuleExports(jvmtiEnv* env,
            jobject module,
            const char* pkg_name,
            jobject to_module) {
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(95);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(95);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
...
static jvmtiError JNICALL
jvmtiTrace_AddModuleOpens(jvmtiEnv* env,
            jobject module,
            const char* pkg_name,
            jobject to_module) {
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(96);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(96);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
...
static jvmtiError JNICALL
jvmtiTrace_AddModuleUses(jvmtiEnv* env,
            jobject module,
            jclass service) {
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(97);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(97);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  service=" PTR_FORMAT "", curr_thread_name, func_name, 
      p2i(service)
    );
  }
...
static jvmtiError JNICALL
jvmtiTrace_AddModuleProvides(jvmtiEnv* env,
            jobject module,
            jclass service,
            jclass impl_class) {
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(98);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(98);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  service=" PTR_FORMAT " impl_class=" PTR_FORMAT "", curr_thread_name, func_name, 
      p2i(service)
    , 
      p2i(impl_class)
    );
...
static jvmtiError JNICALL
jvmtiTrace_IsModifiableModule(jvmtiEnv* env,
            jobject module,
...
#else 
  SafeResourceMark rm;
  jint trace_flags = JvmtiTrace::trace_flags(99);
  const char *func_name = NULL;
  const char *curr_thread_name = NULL;
  if (trace_flags) {
    func_name = JvmtiTrace::function_name(99);
    curr_thread_name = JvmtiTrace::safe_get_current_thread_name();
  }
  if(!JvmtiEnv::is_vm_live()) {
    if (trace_flags) {
...
jvmtiTrace_RetransformClasses(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  class_count=" INT32_FORMAT " classes=" PTR_FORMAT "", curr_thread_name, func_name, class_count, 
      p2i(classes)
    );
  }
...
jvmtiTrace_RedefineClasses(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  class_count=" INT32_FORMAT " class_definitions=" PTR_FORMAT "", curr_thread_name, func_name, class_count, 
...
jvmtiTrace_RawMonitorWait(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  monitor=%s millis=" INT64_FORMAT "", curr_thread_name, func_name, rmonitor->get_name(), millis);
  }
...
jvmtiTrace_SetJNIFunctionTable(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  function_table=" PTR_FORMAT "", curr_thread_name, func_name, 
  p2i(function_table)
);
  }
...
jvmtiTrace_SetEventCallbacks(jvmtiEnv* env,
  ...
  if (Threads::number_of_threads() != 0) {
    ...
    if (size_of_callbacks < 0) {
      if ((trace_flags & JvmtiTrace::SHOW_ERROR) != 0) {
        if ((trace_flags & JvmtiTrace::SHOW_IN) == 0) {
          log_trace(jvmti)("[%s] %s {  callbacks=" PTR_FORMAT "", curr_thread_name, func_name, 
  p2i(callbacks)
);
      }
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  callbacks=" PTR_FORMAT " size_of_callbacks=" INT32_FORMAT "", curr_thread_name, func_name, 
  p2i(callbacks)
, size_of_callbacks);
  }
...
jvmtiTrace_AddCapabilities(jvmtiEnv* env,
  ...
  if ((trace_flags & JvmtiTrace::SHOW_IN) != 0) {
              log_trace(jvmti)("[%s] %s {  capabilities_ptr=" PTR_FORMAT "", curr_thread_name, func_name, 
...
struct jvmtiInterface_1_ jvmtiTrace_Interface = {
      ...
      jvmtiTrace_ResumeThreadList,
                              /*   94 : Add Module Reads */
      jvmtiTrace_AddModuleReads,
                              /*   95 : Add Module Exports */
      jvmtiTrace_AddModuleExports,
                              /*   96 : Add Module Opens */
      jvmtiTrace_AddModuleOpens,
                              /*   97 : Add Module Uses */
      jvmtiTrace_AddModuleUses,
                              /*   98 : Add Module Provides */
      jvmtiTrace_AddModuleProvides,
                              /*   99 : Is Modifiable Module */
      jvmtiTrace_IsModifiableModule,
